...
st.markdown(
    ...
    "##### 3.5T 기준의 신뢰도 검증\n\n"
    "실제 배송 실적(2025년 11월)에 11월 단가표를 적용한 비용과, 3.5T 단일 기준으로 추정한 비용을 비교한 결과:\n\n"
    "| 기준 | 일평균 비용 | 비고 |\n"
    "|---|---:|---|\n"
    "| 실제 (전체 톤수) | 3,969만원 | 3PL 제외(단가표에 해당 톤수 없음) |\n"
    "| 3.5T 추정 | 4,590만원 | VROOM 배차 + 거리 보정 |\n"
    "| **차이** | **+16%** | |\n\n"
    "3.5T 단일 기준 추정값은 실제 대비 약 +16% 수준. "
    "3PL 비용(전체의 32%)이 실제 비용에서 빠져 있어 이를 포함하면 차이는 더 작아짐.\n\n"
    "**시나리오 간 상대 비교에서는 동일 기준을 적용하므로, 절대 오차는 상쇄되어 비교 결과의 유효성에 영향 없음.**"
)
